...
```bash
freeapi help --lang ko
freeapi chat --help --lang en
FREEAPI_LANG=ko freeapi providers --help
```
...
curl -X POST http://127.0.0.1:8080/freeapi/chat \
  -H "Content-Type: application/json" \
  -d '{"message":"follow up","conversation_id":"<이전 응답의 conversation_id>"}'
```
...
freeapi help --lang ko         한국어 도움말 보기
freeapi                        인터랙티브 REPL
...
freeapi server                 로컬 HTTP API 서버 실행
